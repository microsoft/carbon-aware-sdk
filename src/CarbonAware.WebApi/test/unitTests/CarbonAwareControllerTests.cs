...
public class CarbonAwareControllerTests : TestsBase
{
    ...
    ICarbonAwareAggregator carbonAwareAggregator = new Mock<ICarbonAwareAggregator>().Object;
    ...
    public async Task GetEmissionsByMultipleLocations_SuccessfulCallReturnsOk(params string[] locations)
    {
        ...
        var controller = new CarbonAwareController(this.MockCarbonAwareLogger.Object, carbonAwareAggregator, CreateAggregatorWithEmissionsData(data).Object, forecastAggregator);
    ...
    public async Task GetBestEmissions_SuccessfulCallReturnsOk(params string[] locations)
    {
        ...
        var controller = new CarbonAwareController(this.MockCarbonAwareLogger.Object, carbonAwareAggregator, CreateAggregatorWithBestEmissionsData(data).Object, forecastAggregator);
        var parametersDTO = new EmissionsDataForLocationsParametersDTO(){ MultipleLocations = locations };
    ...
    public async Task GetForecast_SuccessfulCallReturnsOk(params string[] locations)
    {
        ...
        var emissionsAggregator = Mock.Of<IEmissionsAggregator>();
        var forecastAggregator = CreateAggregatorWithForecastData(data);
        var controller = new CarbonAwareController(this.MockCarbonAwareLogger.Object, carbonAwareAggregator, emissionsAggregator, forecastAggregator.Object);
        ...
        TestHelpers.AssertStatusCode(result, HttpStatusCode.OK);
        forecastAggregator.Verify(a => a.GetCurrentForecastDataAsync(It.IsAny<CarbonAwareParameters>()), Times.Once);
    }
    ...
    public async Task GetAverageCarbonIntensity_SuccessfulCallReturnsOk(string location, DateTimeOffset start, DateTimeOffset end)
    {
        // Arrange
        double data = 0.7;
        var controller = new CarbonAwareController(this.MockCarbonAwareLogger.Object, carbonAwareAggregator, CreateCarbonAwareAggregatorWithAverageCI(data).Object, forecastAggregator);
    ...
    public async Task CalculateAverageCarbonIntensityBatch_ValidInput()
    {
        ...
        var controller = new CarbonAwareController(this.MockCarbonAwareLogger.Object, carbonAwareAggregator, aggregator.Object, forecastAggregator);
    ...
    {
        var controller = new CarbonAwareController(this.MockCarbonAwareLogger.Object, carbonAwareAggregator, CreateAggregatorWithEmissionsData(new List<EmissionsData>()).Object, forecastAggregator);
