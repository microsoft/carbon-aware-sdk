...
public class SciScoreControllerTests : IntegrationTestingBase
{
    ...
    public async Task SCI_AcceptsValidData_ReturnsContent(string start, string end, string region, HttpStatusCode expectedCode)
    {
        var startTime = DateTimeOffset.Parse(start);
        var endTime = DateTimeOffset.Parse(end);
        _dataSourceMocker.SetupDataMock(startTime, endTime, region);
